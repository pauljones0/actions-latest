...
The updater and maintenance workflow share a concurrency group. Every generated-state push is a normal fast-forward push: concurrent human changes cause failure and require rerunning from current main. No binary conflict merge or force push is allowed. `GH_PAT` supports higher read limits and maintenance workflow-file writes; ordinary data pushes use the workflow token. Secrets are passed only to the steps that need them. Git credentials go through temporary askpass code reading the environment, not token-bearing command arguments or repository configuration.

Weekly maintenance resolves the newest stable release within the current direct tool/action major, refreshes the dependency lock inside declared ranges, updates the scanner version contract, and tests before publication. Transitive versions follow their upstream constraints. Test or resolution failures prevent pushing and are visible in the maintenance job; inspect logs, reproduce with `scripts/maintain.py`, and perform an explicit migration for incompatible changes. To undo a bad accepted maintenance change, revert its commit normally and correct/pin the offending dependency before the next maintenance run. Maintenance dispatches the daily updater explicitly because token-created pushes may not trigger other workflows.
