# actions-latest

Keeping track of the latest versions of various GitHub Actions

https://raw.githubusercontent.com/pauljones0/actions-latest/main/versions.txt

Access that URL for a trusted list of the official Actions belonging to the [GitHub Actions](https://github.com/actions) organization, plus additional actions listed in `trusted-actions.txt`, along with their latest version tags.

The broader community list is published separately:

https://raw.githubusercontent.com/pauljones0/actions-latest/main/community-versions.txt

You can point coding agents such as Claude Code and Codex CLI at this URL so they know the most recent Actions versions to use in their workflow files.

## MCP server

This fork can also run as a stdio MCP server for coding agents:

```json
{
  "mcpServers": {
    "actions-latest": {
      "command": "uvx",
      "args": [
        "--from",
        "git+https://github.com/pauljones0/actions-latest.git",
        "actions-latest-mcp"
      ]
    }
  }
}
```

Available tools:

- `latest_github_actions_versions`: returns trusted `versions.txt` content by default. Pass `include_untrusted=true` to include `community-versions.txt`.
- `latest_github_action_version`: looks up one action, such as `checkout`, `actions/checkout@v4`, or `softprops/action-gh-release@v2`.
- `check_github_actions_workflow`: checks workflow YAML text for outdated GitHub Action references. Pass `include_untrusted=true` to check against the broader community list.

Version recommendations follow the same style as Simon's original `versions.txt`: the highest floating major tag that matches `vN`, such as `actions/checkout@v6`, not exact release tags such as `v6.0.2`. Existing `@stable` branch refs are treated as already intentionally stable.

Set `GITHUB_TOKEN` to avoid GitHub API rate limits when checking actions outside the generated snapshots. Set `ACTIONS_LATEST_URL` to override the trusted versions source, `ACTIONS_LATEST_COMMUNITY_URL` to override the community source, or `ACTIONS_LATEST_OFFLINE=1` to use the packaged snapshots instead of fetching live URLs.

<!-- VERSIONS_START -->
## Latest trusted versions

```
actions/actions-sync@v202601271539
actions/attest@v4
actions/cache@v5
actions/checkout@v6
actions/configure-pages@v6
actions/create-github-app-token@v3
actions/create-release@v1
actions/delete-package-versions@v5
actions/dependency-review-action@v3
actions/deploy-pages@v5
actions/download-artifact@v8
actions/first-interaction@v3
actions/github-script@v9
actions/go-dependency-submission@v2
actions/hello-world-docker-action@v2
actions/hello-world-javascript-action@v1
actions/javascript-action@v1
actions/jekyll-build-pages@v1
actions/labeler@v6
actions/publish-immutable-action@v0
actions/setup-dotnet@v5
actions/setup-elixir@v1
actions/setup-go@v6
actions/setup-haskell@v1
actions/setup-java@v5
actions/setup-node@v6
actions/setup-python@v6
actions/setup-ruby@v1
actions/stale@v10
actions/upload-artifact@v7
actions/upload-code-coverage@v1
actions/upload-pages-artifact@v5
actions/upload-release-asset@v1
astral-sh/ruff-action@v3
astral-sh/setup-uv@v7
github/ai-moderator@v1
github/branch-deploy@v11
github/codeql-action@v4
github/codeql-coding-standards@v2
github/command@v2
github/copilot-release-notes@v1
github/dependabot-action@v3
github/lock@v3
github/update-project-action@v4
github/webpack-bundlesize-compare-action@v1
```
<!-- VERSIONS_END -->

<!-- COMMUNITY_VERSIONS_START -->
## Latest community versions

```
0xProject/setup-helm@v2
1Password/load-secrets-action@v4
2428392/gh-truncate-string-action@v1
a104437ana/sakura-garden@v1
abatilo/actions-poetry@v4
act10ns/slack@v2
actionhippie/install-packwiz@v2
actions-cool/check-user-permission@v2
actions-cool/pr-welcome@v1
actions-ecosystem/action-add-labels@v1
actions-ecosystem/action-create-comment@v1
actions-ecosystem/action-regex-match@v2
actions-ecosystem/action-remove-labels@v1
actions-js/push@v1
actions-rs/audit-check@v1
actions-rs/cargo@v1
actions-rs/clippy-check@v1
actions-rs/toolchain@v1
actions-rust-lang/rustfmt@v1
actions-rust-lang/setup-rust-toolchain@v1
ad-m/github-push-action@v1
Addepar/RedFlag@v1
addnab/docker-run-action@v3
adil192/setup-flutter-submodule@v1
AdityaGarg8/remove-unwanted-software@v5
adrianjost/files-sync-action@v1
advanced-security/filter-sarif@v1
affaan-m/agentshield@v1
agustim/action-hidden-directory@v1
ai-action/ollama-action@v2
airnity/setup-kubeconfig-action@v3
AISquare-Studio/AISquare-Studio-QA@v0
akheron/dependabot-cron-action@v1
aklofas/kicad-happy@v1
alantheprice/ledit-agent@v1
AlbertHernandez/working-label-action@v1
albumprinter/actions-get-git-version@v2
AlexRogalskiy/github-action-tag-replacer@v2
alirezatheh/auto-bump-versions@v1
aliyun/setup-aliyun-cli-action@v1
allure-framework/setup-allurectl@v1
alphayax/appscan-sast-action@v1
alstr/todo-to-issue-action@v5
Amadevus/pwsh-script@v2
amannn/action-semantic-pull-request@v6
amondnet/vercel-action@v42
amp-labs/cli-action@v1
Ana06/get-changed-files@v1
anc95/ChatGPT-CodeReview@v1
anchore/sbom-action@v0
anchore/scan-action@v7
AndreasAugustin/actions-template-sync@v2
andresz1/size-limit-action@v1
Andrew-Chen-Wang/github-wiki-action@v5
android-actions/setup-android@v4
andstor/copycat-action@v3
andstor/file-existence-action@v3
andstor/jsdoc-action@v1
AnimMouse/setup-ffmpeg@v1
AnimMouse/setup-rclone@v1
AnimMouse/setup-youtube-dl@v1
AnimMouse/setup-youtubeuploader@v2
AnimMouse/setup-yt-dlp@v3
AnimMouse/tool-cache@v1
anmol098/waka-readme-stats@v5
anoopt/ms-graph-create-event@v1
anothrNick/github-tag-action@v1
ansible/ansible-content-actions@v1
ansible/ansible-lint@v26
anthonyvscode/newman-action@v1
anthropics/claude-code-action@v1
anyvm-org/cf-tunnel@v0
aormsby/Fork-Sync-With-Upstream-action@v1
AppImageCrafters/build-appimage@v1
Apple-Actions/import-codesign-certs@v7
apple-actions/upload-testflight-build@v5
appleboy/jenkins-action@v1
appleboy/scp-action@v1
appleboy/ssh-action@v1
Arda-cards/deployment-gate-pipeline-action@v1
aredridel/svelte-check-action@v2
ArtiomTr/jest-coverage-report-action@v2
artis3n/ansible_galaxy_collection@v3
artursouza/sticky-pull-request-comment@v2
asdf-vm/actions@v4
ashutoshgngwr/validate-fastlane-supply-metadata@v2
asklokesh/loki-mode@v5
ast-grep/action@v1
ASzc/change-string-case-action@v8
atlassian/gajira-login@v3
atlassian/gajira-transition@v3
attieretief/issue-to-pdf@v1
aurelien-baudet/workflow-dispatch@v4
AurorNZ/paths-filter@v5
AustinScola/mypy-cache-github-action@v1
autofix-ci/action@v1
AutoModality/action-clean@v1
avto-dev/markdown-lint@v1
awalsh128/cache-apt-pkgs-action@v1
aws-actions/amazon-ecr-login@v2
aws-actions/amazon-ecs-deploy-express-service@v1
aws-actions/amazon-ecs-deploy-task-definition@v2
aws-actions/amazon-ecs-render-task-definition@v1
aws-actions/amazon-eks-fargate@v0
aws-actions/application-observability-for-aws@v1
aws-actions/aws-cloudformation-github-deploy@v2
aws-actions/aws-codebuild-run-build@v1
aws-actions/aws-devicefarm-browser-testing@v3
aws-actions/aws-devicefarm-mobile-device-testing@v3
aws-actions/aws-lambda-deploy@v1
aws-actions/aws-secretsmanager-get-secrets@v3
aws-actions/closed-issue-message@v2
aws-actions/configure-aws-credentials@v6
aws-actions/setup-sam@v3
aws-actions/stale-issue-cleanup@v6
aws-actions/sustainability-scanner@v1
aws-actions/vulnerability-scan-github-action-for-amazon-inspector@v1
Azure/aca-review-apps@v0
Azure/aci-deploy@v1
Azure/acr-build@v1
Azure/aks-set-context@v5
Azure/aml-compute@v1
Azure/aml-deploy@v1
Azure/aml-run@v1
Azure/aml-workspace@v1
Azure/appservice-build@v3
Azure/appservice-settings@v1
Azure/arm-deploy@v2
Azure/artifact-signing-action@v2
Azure/bicep-deploy@v2
Azure/build-vm-image@v0
azure/CLI@v3
Azure/container-apps-deploy-action@v2
Azure/docker-login@v2
Azure/functions-action@v1
Azure/functions-container-action@v1
Azure/k8s-bake@v4
Azure/k8s-create-secret@v6
Azure/k8s-deploy@v6
Azure/k8s-lint@v4
Azure/k8s-set-context@v5
Azure/load-testing@v1
Azure/login@v3
Azure/pipelines@v1
Azure/policy-compliance-scan@v0
Azure/powershell@v3
Azure/setup-helm@v5
Azure/setup-kubectl@v5
Azure/sql-action@v2
Azure/static-web-apps-deploy@v1
azure/trusted-signing-action@v2
Azure/webapps-deploy@v3
babbel/publish-npm@v1
bancolombia/checkout-azure-repo@v1
baptiste0928/cargo-install@v3
bcanseco/github-contribution-graph-action@v2
beacon-biosignals/download-run-attempt-artifact@v1
beacon-biosignals/julia-project-dependencies@v1
beacon-biosignals/matrix-output@v1
bearer/bearer-action@v2
benc-uk/workflow-dispatch@v1
benjefferies/branch-protection-bot@v1
benmatselby/gollum-page-watcher-action@v1
BetaHuhn/deploy-to-vercel-action@v1
BetaHuhn/do-spaces-action@v2
BetaHuhn/repo-file-sync-action@v1
betatesterL/aulas@v2
bflad/action-milestone-comment@v2
biomejs/setup-biome@v2
bitovi/github-actions-react-to-github-pages@v1
bitshifted/git-auto-semver@v2
BitskiCo/cargo-cache-action@v1
bitwarden/gh-actions@v0
bloodf/ci-badges-action@v1
BloopAI/apple-code-sign-action@v1
bluefireteam/flutter-gh-pages@v9
bluefireteam/melos-action@v3
bluwy/auto-branch-sync-action@v1
boasiHQ/interactive-inputs@v2
bobheadxi/deployments@v1
bobheadxi/gobenchdata@v1
booxmedialtd/ws-action-parse-semver@v1
Borales/actions-yarn@v5
boundfoxstudios/action-unassign-contributor-after-days-of-inactivity@v1
bridgecrewio/checkov-action@v12
bridgecrewio/yq@v3
browser-actions/setup-chrome@v2
browser-actions/setup-firefox@v1
bufbuild/buf-action@v1
buildjet/cache@v4
buildpulse/buildpulse-action@v3
buildsville/list-pull-requests@v1
bump-sh/github-action@v1
Burnett01/rsync-deployments@v8
burrunan/gradle-cache-action@v3
by-miles/bymiles-pipeline-trigger@v1
bytecodealliance/actions@v1
c-hive/fresh-bot@v1
c-hive/gha-remove-artifacts@v1
cachix/cachix-action@v17
cachix/haskell-release-action@v1
cachix/install-nix-action@v31
cakevm/huff-neo-toolchain@v1
caltechlibrary/iga@v18
CamiloGarciaLaRotta/watermelon-http-client@v1
canonical/setup-lxd@v1
carlosperate/download-file-action@v2
carlrygart/portainer-stack-deploy@v1
cashapp/activate-hermit@v1
cassiodeveloper/SecScore@v1
catchpoint/workflow-telemetry-action@v2
cbrgm/mastodon-github-action@v2
CemitDigital/trivy-report-issue-action@v1
chabroA/action-append-pr-description@v1
chainguard-dev/digestabot@v1
chartboost/ruff-action@v1
Checkmarx/chainalert-github-action@v1
chickensoft-games/setup-godot@v2
ChilliCream/nitro-schema-publish@v16
choam2426/AI-Algorithm-Mentor@v6
chrnorm/deployment-action@v2
chromaui/action@v17
cirolini/genai-code-review@v2
citation-file-format/cffconvert-github-action@v1
ClickHouse/checkout@v1
climpr/deploy-bicep@v1
cloudflare/pages-action@v1
cloudflare/wrangler-action@v4
cloudposse/github-action-matrix-outputs-write@v1
cloudposse/github-action-terraform-plan-storage@v2
coactions/setup-xvfb@v1
cockroachdb/claude-code-action@v1
codecov/basic-test-results@v1
codecov/codecov-action@v6
codecov/test-results-action@v1
CodeGlide/codeglide-mcpgen@v1
CodelyTV/pr-size-labeler@v1
Codesee-io/codesee-action@v2
codespell-project/actions-codespell@v2
codespell-project/codespell-problem-matcher@v1
CodSpeedHQ/action@v4
colespencer1453/release-notes-action-python@v10
common-workflow-lab/upload-conformance-badges@v1
compas-dev/compas-actions.docversions@v3
conda-incubator/relock-conda@v28
conda-incubator/setup-miniconda@v4
ConorMacBride/install-package@v1
coopTilleuls/action-helm@v6
coverallsapp/github-action@v2
cpina/github-action-push-to-another-repository@v1
CQLabs/setup-dcm@v2
crate-ci/typos@v1
crazy-max/ghaction-chocolatey@v4
crazy-max/ghaction-github-labeler@v6
crazy-max/ghaction-github-pages@v5
crazy-max/ghaction-github-runtime@v4
crazy-max/ghaction-github-status@v5
crazy-max/ghaction-import-gpg@v7
crazy-max/ghaction-upx@v4
crazy-max/ghaction-virustotal@v5
crazy-max/ghaction-xgo@v4
cringdahl/sharepoint-file-upload-action@v1
cross-platform-actions/action@v1
crowdin/github-action@v2
ctrf-io/github-test-reporter@v1
CycloneDX/gh-gomod-generate-sbom@v2
cygwin/cygwin-install-action@v6
cyprieng/github-breakout@v1
czl9707/gh-space-shooter@v2
dart-lang/setup-dart@v1
daryl-miller/custom-github-action@v1
dataaxiom/ghcr-cleanup-action@v1
databricks/run-notebook@v0
davelosert/vitest-coverage-report-action@v2
DavidAnson/markdownlint-cli2-action@v23
davidB/rust-cargo-make@v1
dawidd6/action-download-artifact@v21
dawidd6/action-get-tag@v1
dawidd6/action-homebrew-bump-formula@v7
dawidd6/action-send-mail@v17
daytime-em/changelog-on-pr@v1
dcarbone/install-jq-action@v3
ddev/github-action-setup-ddev@v1
de-vri-es/setup-git-credentials@v2
deepset-ai/notify-slack-action@v1
dennisameling/ftp-upload-action@v2
DenverCoder1/doxygen-github-pages-action@v1
dependabot/fetch-metadata@v3
DependencyTrack/gh-upload-sbom@v3
dephraiim/translate-readme@v1
deployphp/action@v1
depot/build-push-action@v1
depot/setup-action@v1
derberg/npm-dependency-manager-for-your-github-org@v6
dessant/label-actions@v5
dessant/lock-threads@v6
dessant/support-requests@v5
DeterminateSystems/determinate-nix-action@v3
DeterminateSystems/flake-checker-action@v12
DeterminateSystems/flakehub-cache-action@v3
DeterminateSystems/flakehub-push@v6
DeterminateSystems/magic-nix-cache-action@v13
DeterminateSystems/nix-installer-action@v22
DeterminateSystems/push-artifact-ids@v1
DeterminateSystems/update-flake-lock@v28
devhub-tools/coverbot-action@v1
devicons/public-upload-to-imgur@v2
devpolo/awake-action@v1
dflook/tofu-apply@v2
dfreilich/pack-action@v1
dg-infrastructure/pipelines-credentials@v1
digi-wolk/olaudit-action@v1
digicert/code-signing-software-trust-action@v1
digital-blueprint/gitlab-pipeline-trigger-action@v1
diku-dk/install-futhark@v1
Dirrk/terraform-docs@v1
ditotet/pedant@v1
dkershner6/reaction-action@v2
dkershner6/switch-case-action@v1
DNYoussef/codeguard-action@v1
docker-practice/actions-setup-docker@v1
docker/bake-action@v7
docker/build-push-action@v7
docker/github-builder@v1
docker/login-action@v4
docker/metadata-action@v6
docker/scout-action@v1
docker/setup-buildx-action@v4
docker/setup-docker-action@v5
docker/setup-qemu-action@v4
dokku/github-action@v1
dolthub/csv-to-md-table-action@v4
dolthub/pull-request-comment-branch@v4
dolthub/pull-request-comment-trigger@v2
dolthub/upload-release-asset@v2
dominikh/staticcheck-action@v1
dorny/paths-filter@v4
dorny/test-reporter@v3
dprint/check@v1
dromara/issues-translate-action@v1
dsherret/rust-toolchain-file@v1
dtinth/setup-github-actions-caching-for-turbo@v1
dtolnay/rust-toolchain@v1
dwenegar/upload-release-assets@v3
easimon/maximize-build-space@v10
easingthemes/ssh-deploy@v6
ecperth/check-aws-inspector@v3
ecraft/project-updater@v1
ecyshor/translate-yaml@v1
edgarrc/action-7z@v1
editorconfig-checker/action-editorconfig-checker@v2
egor-tensin/setup-gcc@v2
eifinger/setup-rye@v4
eigenbot-app/setup-cli@v1
einaregilsson/beanstalk-deploy@v23
elastic/docs-actions@v1
elastic/oblt-actions@v1
elgohr/Github-Release-Action@v5
elgohr/Publish-Docker-Github-Action@v5
EmbarkStudios/cargo-deny-action@v2
emscripten-core/setup-emsdk@v16
encore-mieux/setup-monorepo-node-app@v1
EndBug/add-and-commit@v10
EndBug/latest-tag@v1
EndBug/version-check@v3
EnricoMi/publish-unit-test-result-action@v2
enriikke/gatsby-gh-pages-action@v2
erlef/setup-beam@v1
errata-ai/vale-action@v2
esamattis/npm-release@v2
esphome/build-action@v7
essink/hello-world-docker-action@v1
eteg/deploy-app-zendesk@v4
ethomson/send-tweet-action@v1
eviden-actions/clean-self-hosted-runner@v1
eviden-actions/get-playwright-version@v1
exions/undo-push@v1
expo/expo-github-action@v8
extractions/setup-just@v4
fabasoad/jsonbin-action@v3
fabasoad/nsfw-detection-action@v3
fabasoad/pascal-action@v1
fabasoad/setup-brainfuck-action@v1
fabasoad/setup-cobol-action@v1
fabasoad/setup-mint-action@v1
fabasoad/translation-action@v4
fabasoad/twilio-voice-call-action@v3
facebook/dotslash-publish-release@v2
facebook/install-dotslash@v2
fal-ai/dbt-cloud-action@v3
fastify/github-action-merge-dependabot@v3
fcakyon/conda-publish-action@v1
federicocarboni/setup-ffmpeg@v3
ffurrer2/extract-release-notes@v3
FirebaseExtended/action-hosting-deploy@v0
fjogeleit/http-request-action@v2
fkirc/skip-duplicate-actions@v5
flatpak/flatpak-github-actions@v6
flatt-security/setup-takumi-guard-npm@v1
flowwer-dev/pull-request-stats@v1
foo-software/lighthouse-check-action@v1
fortran-lang/setup-fortran@v1
fossa-contrib/fossa-action@v3
fossas/fossa-action@v1
foundry-rs/foundry-toolchain@v1
FranzDiebold/github-env-vars-action@v2
fregante/setup-git-user@v2
frenck/action-setup-yq@v1
fsfe/reuse-action@v6
FuelLabs/publish-crates@v1
futureware-tech/simulator-action@v5
fuxingloh/multi-labeler@v5
gabrielfalcao/pyenv-action@v18
game-ci/steam-deploy@v3
Gamesight/slack-workflow-status@v2
garnet-org/action@v2
gaurav-nelson/github-action-markdown-link-check@v1
gautamkrishnar/blog-post-workflow@v1
gcarreno/setup-lazarus@v3
gdl-org/clone@v1
GeekyEggo/delete-artifact@v6
getsentry/action-github-app-token@v3
getsentry/action-release@v3
ggml-org/action-create-release@v1
ggml-org/ccache-action@v1
GitGuardian/ggshield-action@v1
github-community-projects/issue-metrics@v4
github-community-projects/stale-repos@v9
gitleaks/gitleaks-action@v2
go-task/setup-task@v2
goanpeca/setup-miniconda@v2
golang/govulncheck-action@v1
golangci/golangci-lint-action@v9
google-github-actions/auth@v3
google-github-actions/create-cloud-deploy-release@v2
google-github-actions/deploy-appengine@v3
google-github-actions/deploy-cloud-functions@v4
google-github-actions/deploy-cloudrun@v3
google-github-actions/get-gke-credentials@v3
google-github-actions/get-secretmanager-secrets@v3
google-github-actions/release-please-action@v4
google-github-actions/run-gemini-cli@v0
google-github-actions/run-vertexai-notebook@v1
google-github-actions/setup-gcloud@v3
google-github-actions/ssh-compute@v2
google-github-actions/upload-cloud-storage@v3
google/clusterfuzzlite@v1
googleapis/release-please-action@v5
GoogleCloudPlatform/release-please-action@v5
goreleaser/goreleaser-action@v7
Gr1N/setup-poetry@v9
graalvm/setup-graalvm@v1
gradle/actions@v6
gradle/develocity-actions@v1
gradle/gradle-build-action@v3
gradle/issue-management-action@v2
gradle/wrapper-validation-action@v3
grafana/run-k6-action@v1
grafana/setup-k6-action@v1
GrantBirki/json-yaml-validate@v4
gsactions/commit-message-checker@v2
GuillaumeFalourd/setup-windows10-sdk-action@v2
HaaLeo/publish-vscode-extension@v2
Harmon758/postgresql-action@v1
hashgraph-online/skill-publish@v1
hashicorp/action-setup-bob@v2
hashicorp/action-setup-enos@v1
hashicorp/actions-docker-build@v2
hashicorp/actions-generate-metadata@v1
hashicorp/actions-go-build@v1
hashicorp/actions-packaging-linux@v1
hashicorp/actions-set-product-version@v2
hashicorp/actions-slack-status@v2
hashicorp/packer-github-actions@v3
hashicorp/setup-packer@v3
hashicorp/setup-terraform@v4
hashicorp/vault-action@v4
haythem/public-ip@v1
helm/chart-releaser-action@v1
helm/chart-testing-action@v2
helm/kind-action@v1
hendrikmuhs/ccache-action@v1
hoverkraft-tech/compose-action@v2
hramos/needs-attention@v1
huggingface/security-workflows@v1
huggingface/tailscale-action@v1
humbletim/install-vulkan-sdk@v1
humbletim/setup-vulkan-sdk@v1
hustcer/milestone-action@v3
hustcer/setup-nu@v3
IAreKyleW00t/verified-bot-commit@v2
ibiqlik/action-yamllint@v3
ietf-tools/semver-action@v1
iFaxity/wait-on-action@v1
ikalnytskyi/action-setup-postgres@v8
ilammy/msvc-dev-cmd@v1
ilammy/setup-nasm@v1
ilharp/sign-android-release@v2
imjohnbo/issue-bot@v3
imranismail/setup-kustomize@v3
indygreg/apple-code-sign-action@v1
int128/docker-manifest-create-action@v2
invertase/github-action-dart-analyzer@v3
InVisionApp/private-action-loader@v5
j178/prek-action@v2
JamesIves/fetch-api-data-action@v2
JamesIves/github-pages-deploy-action@v4
JamesIves/github-sponsors-readme-action@v1
jandedobbeleer/store-submission@v1
JasonEtco/create-an-issue@v2
jaxxstorm/action-install-gh-release@v1
jayqi/failed-build-issue-action@v1
JC5/firefly-iii-dev@v41
jdx/mise-action@v4
jenkey2011/vuepress-deploy@v1
jfrog/frogbot@v2
jfrog/setup-jfrog-cli@v5
jhipster/actions@v1
jitterbit/get-changed-files@v1
johannesjo/action-electron-builder@v1
JohnnyMorganz/stylua-action@v5
jonelantha/gatsby-s3-action@v4
joseph-montanez/forward-event-action@v2
josStorer/get-current-time@v2
joutvhu/get-release@v1
JS-DevTools/npm-publish@v4
jsdaniell/create-json@v1
jtalk/url-health-check-action@v5
jtmullen/submodule-branch-check-action@v1
julia-actions/julia-runtest@v1
julia-actions/setup-julia@v3
juliangruber/approve-pull-request-action@v2
juliangruber/read-file-action@v1
JustinBeckwith/linkinator-action@v2
jwalton/gh-docker-logs@v2
jwalton/gh-find-current-pr@v1
katyo/publish-crates@v2
keehun/xcode-select@v1
KengoTODA/actions-setup-docker-compose@v1
kevin-david/zipalign-sign-android-release@v2
kewisch/action-matrix-notify@v1
Kira-NT/mc-publish@v3
kishikawakatsumi/xcresulttool@v1
koenrh/dnscontrol-action@v3
Kong/gateway-test-scheduler@v7
Kong/public-shared-actions@v2
korthout/backport-action@v4
KSXGitHub/github-actions-deploy-aur@v1
kt3k/update-pr-description@v2
KyleMayes/install-llvm-action@v2
lando/notarize-action@v2
lhotari/action-upterm@v1
lhotari/sandboxed-trivy-action@v1
lima-vm/lima-actions@v1
linear/linear-release-action@v0
linkerd/dev@v49
liskin/gh-problem-matcher-wrap@v4
loft-sh/github-actions@v1
LouisBrunner/checks-action@v3
ls-lint/action@v2
luckyPipewrench/pipelock@v2
lycheeverse/lychee-action@v2
machulav/ec2-github-runner@v2
madhead/check-gradle-version@v1
madrapps/jacoco-report@v1
magnetikonline/action-golang-cache@v5
mamba-org/setup-micromamba@v3
MansaGroup/nrwl-nx-action@v3
MaoLongLong/actions-starcharts@v1
MarceloPrado/has-changed-path@v1
matbour/setup-sentry-cli@v2
mathieudutour/github-tag-action@v5
matomo-org/github-action-tests@v1
Mattraks/delete-workflow-runs@v2
maxim-lobanov/setup-cocoapods@v1
maxim-lobanov/setup-xamarin@v1
maxim-lobanov/setup-xcode@v1
medyagh/setup-minikube@v0
MeilCli/danger-action@v4
metcalfc/changelog-generator@v4
mgrachev/action-dotenv-linter@v3
mheap/github-action-required-labels@v5
mhils/workflows@v19
micalevisk/last-issue-action@v2
micnncim/action-label-syncer@v1
microsoft/microsoft-partner-center-github-action@v3
microsoft/microsoft-store-apppublisher@v1
microsoft/powerplatform-actions@v1
microsoft/setup-msbuild@v3
microsoft/setup-msstore-cli@v1
mikefarah/yq@v4
mikepenz/action-junit-report@v6
mikepenz/release-changelog-builder-action@v6
miloserdow/capistrano-deploy@v3
MinoruSekine/setup-scoop@v4
mirromutth/mysql-action@v1
mitchellh/vouch@v1
mlugg/setup-zig@v2
mondeja/remove-labels-gh-action@v2
monicahq/workflows@v4
moonrepo/setup-rust@v1
mr-smithers-excellent/docker-build-push@v7
mshick/add-pr-comment@v3
msys2/setup-msys2@v2
mxcl/xcodebuild@v3
mxschmitt/action-tmate@v3
MyAlbum/purge-cache@v2
mymindstorm/setup-emsdk@v16
namespace-actions/upload-artifact@v1
namespacelabs/nscloud-cache-action@v1
namespacelabs/nscloud-checkout-action@v9
namespacelabs/nscloud-setup@v0
namespacelabs/nscloud-setup-buildx-action@v0
ncipollo/release-action@v1
netlify/pr-labeler-action@v1
nick-fields/assert-action@v2
nick-fields/retry@v4
nick-invision/retry@v4
nix-community/cache-nix-action@v7
nixbuild/nix-quick-install-action@v34
nodesource/setup-nsolid@v1
nrwl/nx-set-shas@v5
nttld/setup-ndk@v1
NuGet/login@v1
nuget/setup-nuget@v4
nwtgck/actions-comment-run@v3
nwtgck/actions-netlify@v3
nyrkio/change-detection@v2
obi1kenobi/cargo-semver-checks-action@v2
olafurpg/setup-scala@v14
oleksiyrudenko/gha-git-credentials@v2
ondrejmirtes/github-sponsors-readme-action@v1
OP5dev/TF-via-PR@v13
openai/codex-action@v1
openharmony-rs/setup-ohos-sdk@v1
opentofu/setup-opentofu@v2
oras-project/setup-oras@v2
orhun/git-cliff-action@v4
OSDKDev/lock-issues@v1
oven-sh/setup-bun@v2
ovsds/create-or-update-ref-action@v1
pat-s/always-upload-cache@v3
PaulHatch/semantic-version@v4
peaceiris/actions-gh-pages@v4
peaceiris/actions-hugo@v3
peaceiris/actions-label-commenter@v1
peaceiris/actions-mdbook@v2
peakoss/anti-slop@v0
peter-evans/autopep8@v2
peter-evans/close-issue@v3
peter-evans/commit-comment@v4
peter-evans/create-issue-from-file@v6
peter-evans/create-or-update-comment@v5
peter-evans/create-pull-request@v8
peter-evans/dockerhub-description@v5
peter-evans/enable-pull-request-automerge@v3
peter-evans/find-comment@v4
peter-evans/repository-dispatch@v4
peter-evans/slash-command-dispatch@v5
peter-murray/setup-detekt@v3
peterjgrainger/action-create-branch@v1
pixta-dev/repository-mirroring-action@v1
pkgxdev/setup@v5
Platane/snk@v3
plbstl/first-contribution@v4
pmndrs/docs@v3
pnpm/action-setup@v6
pnpm/setup@v0
posener/goaction@v1
PostHog/posthog-github-action@v1
pozil/auto-assign-issue@v1
preactjs/compressed-size-action@v2
prefix-dev/setup-pixi@v0
prince-chrismc/check-actor-permissions-action@v3
protocolbuffers/protobuf-ci@v5
pullfrog/pullfrog@v0
pulumi/actions@v7
py-cov-action/python-coverage-comment-action@v3
PyO3/maturin-action@v1
pyvista/setup-headless-display-action@v4
qltysh/qlty-action@v2
quarto-dev/quarto-actions@v2
r-lib/actions@v2
r0adkll/sign-android-release@v1
r0adkll/upload-google-play@v1
rancher-eio/read-vault-secrets@v3
RasaHQ/create-comment@v1
raven-actions/actionlint@v2
ravsamhq/notify-slack-action@v2
ReactiveCircus/android-emulator-runner@v2
redhat-actions/oc-installer@v1
redhat-actions/oc-login@v1
redhat-actions/openshift-actions-runner-installer@v1
redhat-actions/push-to-registry@v2
redhat-actions/s2i-build@v2
redhat-plumbers-in-action/advanced-issue-labeler@v3
regclient/actions@v0
relative-ci/agent-action@v3
release-drafter/release-drafter@v7
Renato66/auto-label@v3
renovatebot/github-action@v19
repo-sync/github-sync@v2
repo-sync/pull-request@v2
reproducible-containers/buildkit-cache-dance@v3
requarks/changelog-action@v1
returntocorp/semgrep-action@v1
reviewdog/action-actionlint@v1
reviewdog/action-brakeman@v2
reviewdog/action-cpplint@v1
reviewdog/action-depup@v1
reviewdog/action-detect-secrets@v0
reviewdog/action-eslint@v1
reviewdog/action-golangci-lint@v2
reviewdog/action-hadolint@v1
reviewdog/action-languagetool@v1
reviewdog/action-misspell@v1
reviewdog/action-rubocop@v2
reviewdog/action-shellcheck@v1
reviewdog/action-stylelint@v1
reviewdog/action-template@v1
reviewdog/action-tflint@v1
reviewdog/action-tfsec@v1
reviewdog/action-vint@v1
reviewdog/action-yamllint@v1
rharkor/caching-for-turbo@v1
rhysd/action-setup-vim@v1
richardsimko/update-tag@v1
rickstaa/action-create-tag@v1
rickstaa/empty-issues-closer-action@v1
rickstaa/top-issues-action@v1
ridedott/merge-me-action@v2
rlespinasse/git-commit-data-action@v1
rlespinasse/github-slug-action@v5
robinraju/release-downloader@v1
rossjrw/pr-preview-action@v1
rtCamp/action-deploy-wordpress@v3
rtCamp/action-slack-notify@v2
rui314/setup-mold@v1
ryand56/r2-upload-action@v1
s4u/maven-settings-action@v2
samuelmeuli/action-electron-builder@v1
samuelmeuli/action-snapcraft@v3
samuelmeuli/lint-action@v2
samypr100/setup-dev-drive@v4
sarisia/actions-status-discord@v1
sass/clone-linked-repo@v1
satackey/action-docker-layer-caching@v0
say8425/aws-secrets-manager-actions@v2
sbt/setup-sbt@v1
scaleway/action-scw@v0
schdck/create-env-json@v2
seanmiddleditch/gha-setup-ninja@v6
seemethere/download-artifact-s3@v4
seemethere/upload-artifact-s3@v5
sergeysova/jq-action@v2
sergioramos/yarn-actions@v6
SethCohen/github-releases-to-discord@v1
shivammathur/setup-php@v2
shogo82148/actions-goveralls@v1
shogo82148/actions-setup-mysql@v1
shogo82148/actions-setup-perl@v1
shogo82148/actions-setup-redis@v1
shogo82148/actions-upload-release-asset@v1
Shopify/shopify-cla-action@v1
shrink/actions-docker-extract@v4
Sibz/github-status-action@v1
signpath/github-action-submit-signing-request@v2
sigoden/install-binary@v1
sigstore/cosign-installer@v3
siku2/action-hosting-deploy@v1
SimenB/github-actions-cpu-cores@v3
simple-elf/allure-report-action@v1
skiptools/swift-android-action@v2
skjnldsv/check-actor-permission@v3
skjnldsv/read-package-engines-version-actions@v3
slackapi/slack-github-action@v3
Slashgear/action-check-pr-title@v4
snapcore/action-build@v1
snapcore/action-publish@v1
snok/container-retention-policy@v2
snok/install-poetry@v1
snyk/actions@v1
softprops/action-gh-release@v3
softprops/turnstyle@v3
SonarSource/sonarqube-scan-action@v8
Songmu/tagpr@v1
SpicyPizza/create-envfile@v2
srvaroa/labeler@v1
stefanbuck/github-issue-parser@v3
stefanzweifel/changelog-updater-action@v1
stefanzweifel/git-auto-commit-action@v7
step-security/changed-files@v47
step-security/harden-runner@v2
step-security/wait-for-secrets@v1
streetsidesoftware/cspell-action@v8
subosito/flutter-action@v2
suisei-cn/actions-download-file@v1
super-linter/super-linter@v8
superbrothers/close-pull-request@v3
supercharge/mongodb-github-action@v1
superfly/flyctl-actions@v1
svenstaro/upload-release-action@v2
svenstaro/upx-action@v3
Swatinem/rust-cache@v2
swift-actions/setup-swift@v3
taiki-e/cache-cargo-install-action@v3
taiki-e/checkout-action@v1
taiki-e/create-gh-release-action@v1
taiki-e/install-action@v2
taiki-e/setup-cross-toolchain-action@v1
taiki-e/upload-rust-binary-action@v1
tailscale/github-action@v4
tauri-apps/tauri-action@v0
tcort/github-action-markdown-link-check@v1
technote-space/toc-generator@v4
technote-space/workflow-conclusion-action@v3
techpivot/terraform-module-releaser@v2
terraform-docs/gh-actions@v1
terraform-linters/setup-tflint@v6
tespkg/actions-cache@v1
test-summary/action@v2
tgymnich/fork-sync@v2
thollander/actions-comment-pull-request@v3
tibdex/backport@v2
tibdex/github-app-token@v2
timheuer/base64-to-file@v2
TimonVS/pr-labeler-action@v5
tj-actions/branch-names@v9
tj-actions/changed-files@v47
tj-actions/verify-changed-files@v20
tombi-toml/setup-tombi@v1
tracel-ai/github-actions@v11
treosh/lighthouse-ci-action@v12
trilom/file-changes-action@v1
TriPSs/conventional-changelog-action@v6
trstringer/manual-approval@v1
trunk-io/analytics-uploader@v2
TryGhost/action-deploy-theme@v1
ts-graphviz/setup-graphviz@v2
tsickert/discord-webhook@v1
tspascoal/get-user-teams-membership@v4
tw93/contributors-list@v1
tylermurry/github-pr-landmine@v1
tzkhan/pr-update-action@v2
unknwon/send-email-on-failure@v1
updatecli/updatecli-action@v1
uraimo/run-on-arch-action@v3
useblacksmith/build-push-action@v2
useblacksmith/cache@v5
useblacksmith/run-testbox@v2
useblacksmith/rust-cache@v3
useblacksmith/setup-docker-builder@v1
useblacksmith/setup-node@v5
useblacksmith/setup-python@v6
useblacksmith/setup-uv@v4
useblacksmith/stickydisk@v1
usthe/issues-translate-action@v1
utooland/setup-utoo@v1
vale-cli/vale-action@v2
Vankka/pr-target-branch-action@v3
vedantmgoyal2009/winget-releaser@v2
vedantmgoyal9/winget-releaser@v2
vercel/ai-action@v2
vmactions/freebsd-vm@v1
vmactions/netbsd-vm@v1
vmactions/omnios-vm@v1
vmactions/openbsd-vm@v1
vmactions/solaris-vm@v1
voidzero-dev/setup-vp@v1
volta-cli/action@v5
voxmedia/github-action-slack-notify-build@v2
wagoid/commitlint-github-action@v6
Wandalen/wretry.action@v3
wangyoucao577/go-release-action@v1
WarpBuilds/cache@v1
warpdotdev/oz-agent-action@v1
wasmerio/setup-wasmer@v3
wdzeng/edge-addon@v2
wearerequired/lint-action@v2
webbertakken/unity-test-runner@v4
wechuli/allcheckspassed@v2
wei/git-sync@v3
whoan/docker-build-with-cache-action@v8
whywaita/actions-cache-s3@v3
withastro/action@v6
wow-actions/potential-duplicates@v1
wpengine/github-action-wpe-site-deploy@v3
wshihadeh/docker-deployment-action@v2
WyriHaximus/github-action-create-milestone@v1
WyriHaximus/github-action-get-previous-tag@v2
WyriHaximus/github-action-next-semvers@v1
WyriHaximus/github-action-wait-for-status@v1
wzieba/Firebase-Distribution-Github-Action@v1
xalvarez/prevent-file-change-action@v3
XAMPPRocky/create-release@v1
XAMPPRocky/get-github-release@v1
xen0l/iam-lint@v2
xireiki/channel-post@v2
xt0rted/block-autosquash-commits-action@v2
xt0rted/pull-request-comment-branch@v3
xt0rted/slash-command-action@v2
xt0rted/stylelint-problem-matcher@v1
xu-cheng/latex-action@v4
yaananth/run-notebook@v2
YakDriver/md-check-links@v1
YiiGuxing/close-issue@v2
yizhoumo/setup-ossutil@v2
zerotier/github-action@v1
zgosalvez/github-actions-ensure-sha-pinned-actions@v5
zulip/github-actions-zulip@v2
zyborg/gh-action-buildnum@v2
zzak/action-discord@v8
```
<!-- COMMUNITY_VERSIONS_END -->
